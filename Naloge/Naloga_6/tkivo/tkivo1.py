...
s2 = np.sum(1/dfs**2); s2x1 = np.sum(1/dfs**2 * 1/xs); s2x2 = np.sum(1/dfs**2 * 1/xs**2)
M = np.ones((2,2))
M[0,0] = s2x2; M[1,0] = -s2x1; M[0,1] = -s2x1; M[1,1] = s2; M = M / (s2*s2x2 - s2x1**2)
...
ax1.set_title(f"Analitično prilagajanje: $\\chi^2 = {hi2:0.2f}$")
ax1.set_xlabel("x")
ax1.set_ylabel("y")
