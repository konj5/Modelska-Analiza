...
def F(c):
    ...
    return (N0 * np.exp(-L0*np.sqrt(ts)) + N1 * np.exp(-L1*np.sqrt(ts))  - ys)

sol = least_squares(F, x0=[10000,0.01, 10000, 0], method='lm')
...
def f(x):
    return N0 * np.exp(-L0*np.sqrt(x)) + N1 * np.exp(-L1*np.sqrt(x))   
