...
#BASIC ENORAZDELČNI MODEL + CONST (SQRT)
...
def F(c):
    ...
    return (N0 * np.exp(-L0*np.sqrt(ts)) + C0 - ys)

sol = least_squares(F, x0=[10000,0.01,2000], method='lm')
...
def f(x):
    return N0 * np.exp(-L0*np.sqrt(x)) + C0
