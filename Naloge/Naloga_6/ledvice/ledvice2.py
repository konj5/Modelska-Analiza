...
def F(c):
    ...
    return (N0 * np.exp(-L0*ts) + N1 * np.exp(-L1*ts)  - ys)
...
def f(x):
    return N0 * np.exp(-L0*x) + N1 * np.exp(-L1*x)   
...
ax1.set_title("$N_0 \\cdot e^{-L_0t} + N_1 \\cdot e^{-L_1t}$")
ax1.set_xticks([]);ax1.set_yticks([])
print(f"$N_0 = {N0:0.2f}$, $L_0={L0}$  $\\chi^2 = {hi2:0.2f}$")

plt.tight_layout()
plt.savefig("ledvice2.png")
