...
result = np.polyfit([i for i in range(len(data2))], data2, deg=1)
lincoefs = result
lindata = [lincoefs[0] * i + lincoefs[1] for i in range(len(data2))]

result = np.polyfit([i for i in range(len(data2))], data2, deg=2)
quadcoefs = result
quaddata = [quadcoefs[0] * i**2 + quadcoefs[1] * i + quadcoefs[2] for i in range(len(data2))]

lindif = data2 - lindata
quaddif = data2 - quaddata


plt.plot(data2, alpha = 0.5, label = "co2.dat")
plt.plot(lindata, label  = "linearen model")
plt.plot(quaddata, label  = "kvadraten model")
plt.legend()
plt.show()
...
fr, da = spectreFFT(data2)
axs[0].plot(fr, da, label = "fft", color = "black")
axs[1].plot(fr, da, label = "fft", color = "black")
axs[1].set_yscale("log")
...
for order in ords:
    fr, da = spectre(data2,order)
    axs[0].plot(fr, da)
    axs[1].plot(fr, da, label = f"k = {order}")
    axs[1].set_yscale("log")


axs[1].set_xlabel("Frekvenca")
axs[1].set_ylabel("Normirana moč")

axs[0].set_xlabel("Frekvenca")
axs[0].set_ylabel("Normirana moč")

axs[1].legend()

"""poles = poles1(data2, ords)
#axs[2].scatter([1000], [0])
for i,poledata in enumerate(poles):
    axs[2].scatter(poledata[0], poledata[1], label = f"{ords[i]}")

x = np.linspace(-1.0, 1.0, 100)
y = np.linspace(-1.0, 1.0, 100)
X, Y = np.meshgrid(x,y)
F = X**2 + Y**2 - 1
axs[2].contour(X,Y,F,[0])
