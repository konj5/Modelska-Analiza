...
ffr, fda = spectreFFT(data2)



ords = np.arange(4,100,1)

def compare(f,g,x1,x2):
    diff = np.zeros_like(f)
    for i in range(len(f)):
        mini = np.argmin(np.abs(x2-x1[i]))
        diff[i] = np.abs(f[i]-g[mini])
...
for order in ords:
    ...
    naejfnsd.append(np.average(compare(fr,ffr, da, fda)))

plt.plot(ords, naejfnsd, label = "val2.dat")
...
plt.plot(ords, naejfnsd, label = "val3.dat")
...
plt.plot(ords, naejfnsd, label = "co2.dat")
...
#plt.yscale("log")
plt.xlabel("k")
plt.ylabel("Razlika normiranih moči")
