...
plt.plot(np.linspace(0,tmax,len(x1[0,:])), np.sqrt(np.sum(x1*n1**2, axis=0) - np.sum(x1*n1, axis=0)**2), label = "$Matrike: \\beta_r = 0$, $\\beta_s = 1$", linestyle = "dashed")

plt.plot(np.linspace(0,tmax,len(x3[:,0])), np.std(x3, axis=1), label = "$Direktno: \\beta_r = 0$, $\\beta_s = 1$")

plt.plot(np.linspace(0,tmax,len(x2[0,:])), np.sqrt(np.sum(x2*n2**2, axis=0) - np.sum(x2*n2, axis=0)**2), label = "$Matrike: \\beta_r = 4$, $\\beta_s = 5$", linestyle = "dashed")

plt.plot(np.linspace(0,tmax,len(x4[:,0])), np.std(x4, axis=1), label = "$Direktno: \\beta_r = 0$, $\\beta_s = 1$")

plt.legend()

plt.xlabel("Čas")
plt.ylabel("Standardna deviacija populacije")
